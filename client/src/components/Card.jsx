...
function Card({ prompt, photo, name, _id }) {
  return (
    <div className="rounded-xl group relative  shadow-card hover:shadow-cardhover card">
      ...
      <div className="group-hover:flex flex-col max-h-[94.5%] hidden absolute bottom-0 left-0 right-0 bg-[#10131f] m-2 p-4 rounded-md">
        ...
        <div className="mt-5 flex justify-between items-center gap-2">
          ...
          <button type="button" onClick={()=> ImageDownload(_id, photo)} className="outline-none bg-transparent border-none"><img src={DownloadIcon} className="w-6 h-6 object-contain invert" alt="download" /></button>
        </div>
      </div>
    </div>
  );
